...
async def _async_update_ssdp_locations(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    for ssdp_st, key in (
        ...
        for discovery_info in await ssdp.async_get_discovery_info_by_st(hass, ssdp_st):
            ...
            LOGGER.debug("SSDP Location %s: %s", ssdp_st, location)
            host = urlparse(location).hostname
            if host == entry.data[CONF_HOST]:
                updates[key] = location
                break

    updates[CONF_SSDP_RENDERING_CONTROL_LOCATION] = "http://192.168.1.113:9197/dmr"
    updates[CONF_SSDP_MAIN_TV_AGENT_LOCATION] = "http://192.168.1.113:7677/MainTVServer2/"
    if updates:
